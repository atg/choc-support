...
if [[ -e "$CHOC_APP_PATH" ]]; then
	cd "$CHOC_PROJECT_DIR" && HOME=$HOME CHOC_PROJECT_DIR=$CHOC_PROJECT_DIR CHOC_FILE=$CHOC_FILE CHOC_PROJECT_FILE=$CHOC_PROJECT_FILE "$CHOC_APP_PATH/Contents/MacOS/Chocolat" --pretend-node ~/Library/Caches/com.chocolatapp.Chocolat/ohchoc/js-building
fi